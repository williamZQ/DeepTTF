...
# def Z_Score(matrix):
#     mean, std = np.max(matrix), np.min(matrix)
#
#     return (matrix - std) / (mean - std), mean, std
...
def MSE(v, v_):
    ...
    """
    return torch.mean(torch.abs(v_ - v) ** 2)
...
def R2(v, v_):
    ...
    :param v: np.ndarray or int, ground truth.
    :param v_: np.ndarray or int, prediction.
    :return: int, R2 averages on all elements of input.
    """
    return 1 - MSE(v, v_)/torch.var(v)
